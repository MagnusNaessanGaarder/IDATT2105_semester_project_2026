...
const isCompleted = (c: Checklist) => c.status === 'completed' || clientCompletedIds.value.has(c.id)
...
const toggleTask = async (checklist: Checklist, taskId: number) => {
  ...
  if (!checklist.runId || !task.runItemId) return
        ...
        <article class="checklist-card">
          ...
          <div class="checklist-header">
            <div
              ...
              role="button"
              tabindex="0"
              :aria-expanded="String(expandedId === checklist.id)"
            ...
            >
              ...
              <div class="checklist-header__progress" :class="`checklist-header__progress--${tone(completionForChecklist(checklist))}`">
                <div
...
.checklist-header__content {
  ...
  gap: 0.75rem; cursor: pointer; user-select: none; min-width: 0;
  border-radius: var(--radius-sm); padding: 0.25rem; margin: -0.25rem;
  transition: background 0.15s;
}
.checklist-header__content:hover { background: color-mix(in srgb, var(--ik-mat-primary) 6%, transparent); }
.checklist-header__content:focus-visible { outline: 2px solid var(--ik-mat-primary); outline-offset: 2px; border-radius: var(--radius-sm); }
.checklist-header__info { display: flex; flex-direction: column; gap: 0.2rem; min-width: 0; }
...
.progress-track { height: 0.6rem; flex: 1; background: var(--color-gray-200); border-radius: 999px; overflow: hidden; }
.progress-track__fill { height: 100%; background: var(--ik-mat-primary); transition: width 0.3s; }
.progress-pct { font-size: var(--font-size-xs); font-weight: 600; color: var(--color-gray-600); min-width: 2.5rem; text-align: right; }

/* Progress colour levels */
.checklist-header__progress--l1 .progress-track__fill { background: #ef4444; }
.checklist-header__progress--l2 .progress-track__fill { background: #f97316; }
.checklist-header__progress--l3 .progress-track__fill { background: #eab308; }
.checklist-header__progress--l4 .progress-track__fill { background: #84cc16; }
.checklist-header__progress--l5 .progress-track__fill { background: #16a34a; }

/* Status badge */
.status-badge { font-size: 0.65rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; padding: 0.15rem 0.5rem; border-radius: 999px; white-space: nowrap; }
.status-badge--completed { background: color-mix(in srgb, #16a34a 15%, transparent); color: #16a34a; }
.status-badge--pending   { background: color-mix(in srgb, #d97706 15%, transparent); color: #d97706; }
.status-badge--overdue   { background: color-mix(in srgb, var(--color-danger) 15%, transparent); color: var(--color-danger); }

/* Admin action buttons */
.card-actions { display: flex; align-items: center; gap: 0.35rem; flex-shrink: 0; }
.card-action-btn {
  width: 3rem; height: 3rem;
  display: inline-flex; align-items: center; justify-content: center;
  border: 1px solid var(--color-border);
  background: color-mix(in srgb, var(--color-gray-100, #f3f4f6) 60%, var(--color-card));
  border-radius: var(--radius-md); cursor: pointer; color: var(--color-gray-600);
  flex-shrink: 0; padding: 0; transition: background 0.15s, color 0.15s, border-color 0.15s;
}
...
.task-row {
  ...
  background: var(--color-card); transition: background 0.12s, border-color 0.12s;
}
.task-row:hover { background: color-mix(in srgb, var(--ik-mat-primary) 4%, var(--color-card)); border-color: color-mix(in srgb, var(--ik-mat-primary) 40%, var(--color-border)); }
.task-row--deviation { border-color: #d97706; background: color-mix(in srgb, #d97706 6%, var(--color-card)); }

/* Label fills entire row */
.task-label { display: flex; gap: 0.65rem; align-items: flex-start; cursor: pointer; min-height: 2.75rem; padding: 0.55rem 0.75rem; width: 100%; }
.task-label input[type="checkbox"] { width: 1.15rem; height: 1.15rem; flex-shrink: 0; margin-top: 0.1rem; accent-color: var(--ik-mat-primary); cursor: pointer; }
.task-label__body { display: flex; flex-direction: column; gap: 0.2rem; flex: 1; min-width: 0; }
.task-label__body > span { font-size: var(--font-size-sm); color: var(--color-foreground); }
.task-done { color: var(--color-gray-400) !important; text-decoration: line-through; }
.req-mark { color: var(--color-danger); font-weight: bold; margin-left: 0.1rem; }
.task-meta { display: flex; flex-wrap: wrap; gap: 0.35rem; }
.deviation-badge { font-size: 0.65rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.04em; color: #d97706; background: color-mix(in srgb, #d97706 12%, transparent); border-radius: 999px; padding: 0.1rem 0.4rem; }
.task-notes { font-size: var(--font-size-xs); color: var(--color-gray-500); }
.no-tasks { margin: 0; font-size: var(--font-size-sm); color: var(--color-gray-500); font-style: italic; }

/* Bottom action row */
.action-row { display: flex; align-items: center; justify-content: flex-end; gap: 0.75rem; padding-top: 0.25rem; border-top: 1px solid var(--color-border); flex-wrap: wrap; min-height: 2rem; }
.action-error { margin: 0; font-size: var(--font-size-xs); color: var(--color-danger); flex: 1; }
.action-hint { font-size: var(--font-size-xs); color: var(--color-gray-500); font-style: italic; }

.empty-state { margin-top: 0.9rem; border: 1px solid var(--color-border); border-radius: var(--radius-md); background: var(--color-card); color: var(--color-gray-600); text-align: center; padding: 1.2rem; }

/* Buttons */
.btn { min-height: 2.5rem; padding: 0 1rem; border-radius: var(--radius-md); font-size: var(--font-size-sm); font-weight: 600; cursor: pointer; border: none; display: inline-flex; align-items: center; transition: opacity 0.15s; }
.btn:disabled { opacity: 0.45; cursor: not-allowed; }
.btn--primary { background: var(--ik-mat-primary); color: #fff; }
.btn--primary:hover:not(:disabled) { opacity: 0.9; }
.btn--ghost { background: transparent; border: 1px solid var(--color-border); color: var(--color-gray-700); }
.btn--ghost:hover { background: var(--color-accent); }
.btn--danger { background: var(--color-danger); color: #fff; }
.btn--danger:hover:not(:disabled) { opacity: 0.88; }
.btn--sm { min-height: 2.75rem; font-size: var(--font-size-xs); padding: 0 0.9rem; }
.btn:focus-visible { outline: 2px solid var(--ik-mat-primary); outline-offset: 2px; }

/* Modal */
.modal-form { display: flex; flex-direction: column; gap: 1rem; }
.form-group { display: flex; flex-direction: column; gap: 0.25rem; }
.form-group label { font-size: var(--font-size-sm); font-weight: 500; color: var(--color-gray-700); }
.form-group input, .form-group textarea, .form-group select { min-height: 2.5rem; padding: 0.5rem 0.75rem; border: 1px solid var(--color-border); border-radius: var(--radius-md); font-size: var(--font-size-sm); font-family: inherit; }
.form-group input:focus, .form-group textarea:focus, .form-group select:focus { outline: none; border-color: var(--ik-mat-primary); }
.form-group textarea { min-height: 4rem; resize: vertical; }
.modal-error { margin: 0; flex: 1; font-size: var(--font-size-sm); color: var(--color-danger); }
.tasks-header { display: flex; align-items: center; justify-content: space-between; }
.add-task-btn { font-size: var(--font-size-xs); font-weight: 600; color: var(--ik-mat-primary); background: none; border: 1px dashed var(--ik-mat-primary); border-radius: var(--radius-sm); padding: 0.25rem 0.6rem; cursor: pointer; }
.add-task-btn:hover { background: color-mix(in srgb, var(--ik-mat-primary) 8%, transparent); }
.tasks-empty { font-size: var(--font-size-xs); color: var(--color-gray-500); font-style: italic; margin: 0.25rem 0 0; }
.tasks-edit-list { list-style: none; margin: 0.4rem 0 0; padding: 0; display: flex; flex-direction: column; gap: 0.35rem; }
.task-edit-row { display: flex; align-items: center; gap: 0.5rem; background: var(--color-gray-50, #f9fafb); border: 1px solid var(--color-border); border-radius: var(--radius-sm); padding: 0.35rem 0.5rem; }
.task-edit-input { flex: 1; border: none; background: transparent; font-size: var(--font-size-sm); font-family: inherit; outline: none; min-width: 0; }
.task-req-label { display: flex; align-items: center; gap: 0.25rem; font-size: var(--font-size-xs); color: var(--color-gray-600); white-space: nowrap; flex-shrink: 0; }
.remove-task-btn { background: none; border: none; color: var(--color-gray-400); font-size: 1.1rem; cursor: pointer; line-height: 1; padding: 0 0.15rem; flex-shrink: 0; }
.remove-task-btn:hover { color: var(--color-danger); }
.delete-text { margin: 0; font-size: var(--font-size-sm); color: var(--color-gray-700); line-height: 1.6; }

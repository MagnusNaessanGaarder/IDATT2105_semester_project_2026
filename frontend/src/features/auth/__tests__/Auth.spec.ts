...
describe('Auth Store', () => {
    ...
    describe('Register', () => {
        it('should successfully register and store tokens', async () => {
            ...
            expect(store.isAuthenticated).toBe(true)
            expect(sessionStorage.getItem('accessToken')).toBe('register-access-token')
            expect(sessionStorage.getItem('refreshToken')).toBe('register-refresh-token')
            expect(sessionStorage.getItem('email')).toBe('new@everest-sushi.no')
            expect(sessionStorage.getItem('role')).toBe('EMPLOYEE')
        })

        it('should forward fullName to authApi.register', async () => {
            vi.mocked(authApi.register).mockResolvedValue({
                accessToken: 'tok', refreshToken: 'ref', email: 'a@b.no', role: 'EMPLOYEE', organizations: [],
            })

            const store = useAuthStore()
            await store.register({ email: 'a@b.no', password: 'Test1234!', fullName: 'Ola Nordmann' })

            expect(authApi.register).toHaveBeenCalledWith(
                expect.objectContaining({ fullName: 'Ola Nordmann' })
            )
        })

        it('should forward phone to authApi.register when provided', async () => {
            vi.mocked(authApi.register).mockResolvedValue({
                accessToken: 'tok', refreshToken: 'ref', email: 'a@b.no', role: 'EMPLOYEE', organizations: [],
            })
        ...
        it('should pass undefined phone when omitted', async () => {
            vi.mocked(authApi.register).mockResolvedValue({
                accessToken: 'tok', refreshToken: 'ref', email: 'a@b.no', role: 'EMPLOYEE', organizations: [],
            })
        ...
        it('should set loading true during registration and false after', async () => {
            let resolve!: () => void
            vi.mocked(authApi.register).mockReturnValue(
                new Promise((r) => {
                    resolve = () => r({ accessToken: 't', refreshToken: 'r', email: 'e@e.no', role: 'EMPLOYEE', organizations: [] })
                })
            )

            const store = useAuthStore()
            const promise = store.register({ email: 'e@e.no', password: 'Test1234!', fullName: 'Test' })
        ...
        it('should clear loading and set error on failure', async () => {
            vi.mocked(authApi.register).mockRejectedValue(new Error('Server error'))

            const store = useAuthStore()
            await expect(
                store.register({ email: 'x@x.no', password: 'Test1234!', fullName: 'Test' })
            ).rejects.toBeTruthy()
        ...
        it('should map backend field errors on register failure', async () => {
            ...
            const store = useAuthStore()
            await expect(
                store.register({ email: 'x', password: 'x', fullName: 'Test' })
            ).rejects.toBeTruthy()
        ...
        it('should not be authenticated after failed registration', async () => {
